...
function WeatherSummary({
  ...
  const conditionLabel =
    copy.weatherConditionLabels[weather.current.iconKey] ??
    copy.weatherConditionLabels.unknown;
  const attribution =
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-600">
      <span className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 font-medium text-slate-800">
        <span
          aria-hidden="true"
          className={[
...
function PreviewCard({
  ...
  return (
    <div className="w-72 max-w-[calc(100vw-3rem)] rounded-2xl border border-slate-200 bg-white/95 p-4 text-left shadow-xl shadow-slate-900/15 backdrop-blur">
      ...
      <WeatherSummary weather={weather} locale={locale} copy={copy} />
